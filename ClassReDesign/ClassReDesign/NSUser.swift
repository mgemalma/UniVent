...
class NSUser: NSObject, NSCoding {
    ...
    private var pEvents: [Int]?         // Stores all posted Events as IDs.
    private var aEvents: [Int]?         // Stores all attending Events as IDs.
    ...
    static func getPostedEvents() -> [Int]? { return user.pEvents }
    static func getAttendingEvents() -> [Int]? { return user.aEvents }
    ...
    static func setPostedEvents(pEvents: [Int]) { user.pEvents = pEvents }
    static func setAttendingEvents(aEvents: [Int]) { user.aEvents = aEvents }
    ...
    static func loadDB(id: String) -> Bool {
        ...
        let peArr: [String] = arrayer(string: dict!["pEvents"]) as! [String]
        user.pEvents = peArr.map { Int($0) ?? 0 }
        let aeArr: [String] = arrayer(string: dict!["aEvents"]) as! [String]
        user.aEvents = aeArr.map { Int($0) ?? 0 }
