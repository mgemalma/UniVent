...
class NSUserTest: NSObject, NSCoding {
    ...
    static let arcURL = docDir.appendingPathComponent("userTestDisk")
     ...
     **/
    static var user: NSUserTest = {
        // Create Instance
        let instance = NSUserTest()
        // Return Instance
        return instance
    }()
     ...
     **/
    var id: String?             // Stores id assigned by FB.
    var name: String?           // Stores name associated with id.
    var flags: Int?             // Stores flag count of the user.
    var rad: Float?             // Stores the preffered search radius.
    var interests: [String]?    // Stores the list of Interests of the user. Should change to Enum.
    var pEvents: [String]?      // Stores all posted Events as IDs.
    var aEvents: [String]?      // Stores all attending Events as IDs.
    var rEvents: [String]?      // Stores all posted Events as IDs.
    var fEvents: [String]?      // Stores all posted Events as IDs.
    var loc: CLLocation?
    ...
    {
        if let sharedUser = NSKeyedUnarchiver.unarchiveObject(withFile: arcURL.path) as? NSUserTest {
            user = sharedUser
            print("Success")
            return true
        }
